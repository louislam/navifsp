...
function findMSBuild(): string {
    ...
    const vs = "C:\\Program Files (x86)\\Microsoft Visual Studio\\Installer\\vswhere.exe";
    if (existsSync(vs)) {
        const r = spawnSync(vs, ["-products", "*", "-find", "MSBuild\\**\\Bin\\MSBuild.exe", "-latest"], { encoding: "utf-8", stdio: "pipe" });
        if (r.status === 0) {
